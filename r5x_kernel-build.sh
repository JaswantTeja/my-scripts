...
ARCH=arm64
export ARCH
KBUILD_BUILD_HOST=blush-caf
export KBUILD_BUILD_HOST
KBUILD_BUILD_USER="JaswantTeja"
...
zipping() {
	cd AnyKernel || exit 1
	zip -r9 Blush-Test-CAF-"${BRANCH}"-KERNEL-"${CODENAME}"-"${DATE}".zip ./*
	curl -sL https://git.io/file-transfer | sh
	./transfer wet Blush-Test-CAF-"${BRANCH}"-KERNEL-"${CODENAME}"-"${DATE}".zip
	cd ..
}
